...
def naiveSoftmaxLossAndGradient(
    ...
    score = np.matmul(outsideVectors, centerWordVec) # center & outside all words dot product // [v,1]
    y_hat = softmax(score) # get probability between 0~1 // [v,1]

    loss = -np.log(y_hat[outsideWordIdx])

    ds = y_hat.copy() # // [v,] 
    ds[outsideWordIdx] -= 1 # y hat - y (y = one hot vector)
    
    gradCenterVec = np.matmul(outsideVectors.T, ds) # U^T * (y hat - y) // [emb,1]
    gradOutsideVecs = np.dot(ds.reshape(-1,1), centerWordVec.reshape(1,-1)) # (y hat - y) * v_c^T // [v,1]*[1,emb] = [v, emb]
...
def negSamplingLossAndGradient(
    ...
    for i in range(K):
        n_idx = indices[i+1] #negative index
        u_k = outsideVectors[n_idx]
        z = sigmoid(-np.dot(u_k,centerWordVec))
        loss -= np.log(z)
        gradCenterVec += (1-z)*u_k
        gradOutsideVecs[n_idx] += (1-z)*centerWordVec
